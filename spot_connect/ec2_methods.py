...
def get_spot_instance(spotid,
    ...
    if not using_instance_id:
        ...
        try:
            ...
            if 'InvalidKeyPair.Duplicate' in str(e):
                ...
                sys.stdout.write(
                    "Was not able to find Key-Pair in default directory " + str(kp_dir))
                sys.stdout.write(
    ...
    if not using_instance_id:
        ...
        if 'security_group' not in profile:
            ...
            sg = iam_methods.get_security_group(client, 'SG-' + spotid, enable_nfs=enable_nfs,
                                                enable_ds=enable_ds, firewall_ingress_settings=profile['firewall_ingress'])

            # For the profile we need a tuple of the security group ID and the security group name.
            # Add the security group ID and name to the profile dictionary
            profile['security_group'] = (sg['GroupId'], 'SG-' + spotid)
    ...
    if using_instance_id:
        ...
        if len(reservations) == 0:
            raise Exception(
                'Unable to find instance with given id. Cannot create instances based on instance-ids. Submit a name to create an instance. Exiting.')
        instance_id = spotid
    ...
    # Otherwise, filter using the instance's launch group
        ...
        if len(spot_requests) > 0:
            ...
            launch_specs = {
                ...
                # do not optimize for EBS storage
                'EbsOptimized': False,
                # AWS image ID. List available programatically or through launch wizard
                'ImageId': profile['image_id'],
                # Instance type. List available programatically or through wizard or at https://aws.amazon.com/ec2/spot/pricing/
                'InstanceType': profile['instance_type'],
                'Placement': {
                    'AvailabilityZone': profile['region'] + profile['zone']
                },
                # Name for the key pair
                'KeyName': profile['key_pair'][0],
                # Enable monitoring
            ...
            response = client.request_spot_instances(
                ...
                # submit a name to ensure idempotency
                ClientToken=spotid,
                # if True, checks if you have permission without actually submitting request
                DryRun=False,
                # number of individual instances
                InstanceCount=1,
                LaunchGroup=spotid,
                LaunchSpecification=launch_specs,
                # Must be greater than current instance type price for region, available at https://aws.amazon.com/ec2/spot/pricing/
                SpotPrice=profile['price'],
                # Persisitence is usually not necessary (given storage backup) or advisable with spot instances
                Type='one-time',
                # Instance terminates if typing `shutdown -h now` in the console
        ...
        while not instance_id:  # I know this is unusual cause its not a boolean but it works.
            spot_req = client.describe_spot_instance_requests(
                Filters=[{'Name': 'spot-instance-request-id', 'Values': [spot_req_id]}])['SpotInstanceRequests']
            ...
            if len(spot_req) > 0:
                ...
                if not spot_tag_added:
                    client.create_tags(Resources=[spot_req['SpotInstanceRequestId']], Tags=[
                                       {'Key': 'Name', 'Value': spotid}])
                    spot_tag_added = True
    ...
    try:
        reservations = client.describe_instances(
            Filters=[{'Name': 'instance-id', 'Values': [instance_id]}])['Reservations']
    ...
    if str(instance['State']['Name']) == 'terminated':
        raise Exception(
            'Desired spot request has been terminated, please choose a new instance name or wait until the terminated spot request has expired in the AWS console')

    instance_status = check_instance_initialization(
        instance_id, client=client, instance_wait_sleep=instance_wait_sleep)

    # Wait until the instance is runing to connect
    if instance_status != 'ok':
        raise Exception('Failed to boot, instance status: %s' %
                        str(instance_status))
...
def check_instance_initialization(instance_id, client=None, region=None, instance_wait_sleep=5):
    ...
    if client is None:
        try:
            ...
            raise Exception('If client is None region must be passed.')
        # Connect to aws ec2 subnet as a client
...
def connect_to_instance(ip, keyfile, username='ubuntu', port=22, timeout=30):
    '''
    ...
    # Create an RSA key from the key file to avoid runtime
    ...
    while connected == False:
        try:
            ...
            ssh_client.connect(ip, username=username, pkey=k,
