...
class SpotInstance:
    ...
    def __init__(self,
        ...
        if filesystem is None:
            ...
            print('Instance will be mounted on the ' +
                  self.filesystem + ' elastic filesystem')
        ...
        if security_group is not None:
            sg = iam_methods.retrieve_security_group(
                security_group, region=self.profile['region'])
        ...
        try:
            ...
            self.instance, self.profile = ec2_methods.get_spot_instance(self.name, self.profile, instance_profile=self.instance_profile, monitoring=self.monitoring,
                                                                        kp_dir=self.kp_dir, using_instance_id=self.using_id)  # Launch or connect to the spot instance under the given name
        ...
        if self.profile['efs_mount']:
            ...
            try:
                self.mount_target, self.instance_dns, self.filesystem_dns = efs_methods.retrieve_efs_mount(
                    fs_name, self.instance, new_mount=self.new_mount, region=self.profile['region'])
            except Exception as e:
                raise e

            print('Connecting instance to link EFS...')
            instance_methods.run_script(self.instance, self.profile['username'], bash_scripts.compose_mount_script(
                self.filesystem_dns), kp_dir=self.kp_dir, cmd=True)
    ...
    def refresh_instance(self, verbose=True):
        ...
        reservations = client.describe_instances(
            InstanceIds=[self.instance['InstanceId']])['Reservations']
    ...
    def upload(self, files, remotepath, verbose=False):
        ...
        instance_methods.upload_to_ec2(
            self.instance, self.profile['username'], files_to_upload, remote_dir=remotepath, kp_dir=self.kp_dir, verbose=verbose)
    ...
    def download(self, files, localpath):
        ...
        if type(localpath) is str:
            ...
            raise TypeError(
                'put must be str or list of str with equal length to `get`')
        ...
        instance_methods.download_from_ec2(
            self.instance, self.profile['username'], files_to_download, put=localpath, kp_dir=self.kp_dir)
    ...
    def run(self, scripts, cmd=False, return_output=False, time_it=False):
        ...
        for script in scripts:
            ...
            try:
                if return_output:
                    run_stat, output = instance_methods.run_script(
                        self.instance, self.profile['username'], script, cmd=cmd, kp_dir=self.kp_dir, return_output=return_output)
                else:
                    run_stat = instance_methods.run_script(
                        self.instance, self.profile['username'], script, cmd=cmd, kp_dir=self.kp_dir, return_output=return_output)
    ...
    def clone_repo(self, repo_link, directory='/home/ec2-user/efs/'):
        ...
        if proceed:
            ...
            raise Exception(str(directory) +
                            ' directory was not found on instance')
    ...
    def update_repo(self, path_on_instance, branch=None, repo_link=None):
        ...
        if proceed:
            command = update_git_repo(
                path_on_instance, branch=branch, repo_link=repo_link, command='')
            self.run(command, cmd=True)
        else:
            raise Exception(str(path_on_instance) +
                            ' path was not found on instance')

    def dir_exists(self, directory):
        '''Check if a directory exists'''
        # This method is critical for the s3 sync to work in the instance manager
        output = self.run(
            '[ -d "' + directory + '" ] && echo "True" || echo "False"', cmd=True, return_output=True)
        if 'True' in output:
            return True
        else:
            return False
