...
def main():                                                     # Main execution
    ...
    parser.add_argument(
        '-n', '--name', help='name for spot instance launch group (will be used as identifier)', default='')
    parser.add_argument('-iid', '--instanceid',
                        help='instance id string (overrides --name)', default='')
    parser.add_argument(
        '-kp', '--keypair', help='name of the key pair to use (will default to KP-<name> if none is submitted)', default='')
    parser.add_argument('-sg', '--securitygroup',
                        help='name of the security group to use (will default to SG-<name> if none is submitted)', default='')
    parser.add_argument('-ip', '--instanceprofile',
                        help='instance profile with attached IAM roles', default='')

    parser.add_argument('-p', '--profile', help='profile with efsmount, firewall, imageid, price, region, availability zone, script and username settings any of which can be set here).',
                        default=list(profiles.keys())[0], choices=profiles.keys())
    parser.add_argument(
        '-em', '--efsmount', help='if True, will connect or create a filesystem (for internal use, if no filesystem name is submitted this will be False)', default=True)
    parser.add_argument(
        '-fw', '--firewall', help='a tuple of len 4 with firewall settings', default='')
    parser.add_argument('-ami', '--imageid',
                        help='the ID for the AMI image to use', default='')
    parser.add_argument(
        '-prc', '--price', help='custom maximum price for the instance', default='')
    parser.add_argument('-reg', '--region',
                        help='AWS Region to use', default='us-east-1')
    parser.add_argument(
        '-az', '--zone', help='AWS Availability Zone to use', default='a')
    parser.add_argument(
    ...
    parser.add_argument(
        '-nm', '--newmount', help='create a new mount target even if one exists (for internal use)', default=False)
    parser.add_argument(
        '-u', '--upload', help='file or directory to upload', default='')
    parser.add_argument(
        '-r', '--remotepath', help='directory on EC2 instance to upload via ordinary NFS', default='.')
    parser.add_argument('-a', '--activeprompt',
                        help='if "True" leave an active shell open after running scripts', default=False)
    parser.add_argument(
        '-t', '--terminate', help='terminate the instance after running everything', default=False)
    parser.add_argument(
        '-m', '--monitoring', help='activate monitoring for the instance', default=True)
    ...
    if args.filesystem != '':
        print('Instance will be mounted on the ' +
              args.filesystem + ' elastic filesystem')
    ...
    if args.securitygroup != '':
        ...
        sg = iam_methods.retrieve_security_group(
            args.securitygroup, region=profile['region'])
    ...
    try:
        ...
        instance, profile = ec2_methods.get_spot_instance(spot_identifier, profile, instance_profile=args.instanceprofile, monitoring=args.monitoring,
    ...
    if args.upload != '':
        ...
        instance_methods.upload_to_ec2(
            instance, profile['username'], files_to_upload, remote_dir=args.remotepath)
