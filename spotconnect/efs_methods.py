...
def launch_efs(system_name, region='us-west-2', launch_wait=3):
    ...
    file_systems = client.describe_file_systems(
        CreationToken=system_name)['FileSystems']
    ...
    if len(file_systems) == 0:
        ...
        while not initiated:
            ...
            try:
                file_system = client.describe_file_systems(
                    CreationToken=system_name)['FileSystems'][0]
                initiated = True
...
def retrieve_efs_mount(file_system_name, instance, new_mount=False, region='us-west-2', mount_wait=3):
    ...
    if new_mount:
        ...
        # Gather the instance subnet ID. Subnets are your personal cloud, for a full explanation see https://docs.aws.amazon.com/vpc/latest/userguide/VPC_Subnets.html
        subnet_id = instance['SubnetId']
        # Get the instance's security group
        security_group_id = instance['SecurityGroups'][0]['GroupId']

        ec2 = boto3.resource('ec2')
        # Get the features of the subnet
        subnet = ec2.Subnet(subnet_id)

        # Get the IPv4 CIDR block assigned to the subnet.
        net = IPNetwork(subnet.cidr_block)
        # The CIDR block is a block or range of IP addresses, we only need to assign one of these to a single mount
        ...
        while not complete:
            try:
                response = client.create_mount_target(                         # Create the mount target
                    # Under the file system just created
                    FileSystemId=file_system_id,
                    # Under the same subnet as the EC2 instance you've just created
                    SubnetId=subnet_id,
                    # Assign it the first IP Adress from the CIDR block assigned to the subnet
                    IpAddress=ips[ipid],
                    SecurityGroups=[
                        # Apply the security group which must have ingress rules to allow NFS client connections (enable port 2049)
                        security_group_id,
                    ]
                )
                complete = True
        ...
        while not initiated:
            try:
                mount_target = client.describe_mount_targets(
...
def get_mounttarget_dns(file_system_name, region, availability_zone):
    ...
    mount_target_dns = availability_zone + '.' + \
