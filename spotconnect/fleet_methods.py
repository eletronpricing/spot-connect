...
def launch_spot_fleet(account_number,
    ...
    if 'key_pair' not in profile:
        if name is None:
            raise Exception(
                'key_pair not in profile. Please use name arg to create a key-pair & security group')
    ...
    if 'security_group' not in profile:
        ...
        sg = iam_methods.get_security_group(client, 'SG-' + name, enable_nfs=enable_nfs,
        ...
        profile['security_group'] = (sg['GroupId'], 'SG-' + name)
