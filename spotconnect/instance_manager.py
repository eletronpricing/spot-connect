...
from spotconnect import sutils
from spotconnect import spotted
from spotconnect.sutils import genrs, load_profiles, split_workloads
from spotconnect.bash_scripts import compose_s3_sync_script
from spotconnect.fleet_methods import launch_spot_fleet, get_fleet_instances
from spotconnect.efs_methods import launch_efs
...
class InstanceManager:
    ...
    def __init__(self, kp_dir=None, efs=None):
        ...
        if kp_dir is None:
            try:
                ...
                print(
                    'You can change the default key-pair directory using spot_connect.sutils.set_default_kp_dir(<dir>)' % kp_dir)
                self.kp_dir = kp_dir
        else:
            self.kp_dir = kp_dir
    ...
    def quick_launch(self, instance_name='monitor', profile='t2.micro'):
        ...
        self.instances[iname] = spotted.SpotInstance(
            iname, profile=profile, filesystem=self.efs, kp_dir=self.kp_dir)
    ...
    def instance_s3_transfer(self, source, dest, instance_profile, efs=None, instance_name=None):
        ...
        if efs is None:
            if self.efs is None:
                answer = (
                    'You have not specified an EFS, the instance will shut down after the sync and your data may not persist. Do you want to continue (Y)? ')
                if answer == "Y":
                    fs = None
                else:
        ...
        self.instances[iname] = spotted.SpotInstance(
            iname, profile='t3.small', filesystem=fs, kp_dir=self.kp_dir, instance_profile=instance_profile)

        # Check whether the s3 bucket is the source or destination
        if 's3://' in source:
            instance_file_exists, instance_path, bucket_path = self.downloader.dir_exists(
                dest), dest, source
        elif 's3://' in dest:
            instance_file_exists, instance_path, bucket_path = self.downloader.dir_exists(
                source), source, dest
            ...
            raise Exception(
                'Either the source or dest must be an S3 bucket formatted as "s3://<bucket name>"')
        ...
        if not instance_file_exists:
            ...
            bucket_path_exists = s3.Bucket(
                bucket_path.replace('s3://', '')) in s3.buckets.all()
        if not bucket_path_exists:
            ...
            command = compose_s3_sync_script(
                source, dest, instance_path, logfile=iname)

        self.instances[iname].run(command, cmd=True)

        print('Files and directories from ' + source +
              ' are being is being synced to ' + dest + ' on the instance "' + iname + '"')
        print('The instance will be shutdown and terminated when the job is complete.')
        print('Use the following to check progress: SpotInstance("' + iname +
              '").run("cat ' + instance_path + '/' + iname + '.txt", cmd=True)')
    ...
    def launch_fleet(self,
        ...
        fleet_instances = get_fleet_instances(
            spot_fleet_req_id, region=profile['region'])
    ...
    def refresh_fleet_instances(self, fleet_id=None, region=None):
        ...
        if fleet_id is not None:
            ...
            if region is None:
                self.fleets[fleet_id]['instances'] = get_fleet_instances(
                    fleet_id, self.fleets[fleet_id]['region'])
            else:
                self.fleets[fleet_id]['instances'] = get_fleet_instances(
                    fleet_id, region)
        for fleet in self.fleets:
            if region is None:
                self.fleets[fleet]['instances'] = get_fleet_instances(
                    fleet, self.fleets[fleet]['region'])
            else:
                self.fleets[fleet_id]['instances'] = get_fleet_instances(
                    fleet_id, region)
    ...
    def run_distributed_jobs(self, account_number, prefix, n_jobs, profile, availability_zone=None, user_data=None, instance_profile=''):
        ...
        for nn in range(n_jobs):
            ...
            if user_data is None:
                self.launch_fleet(account_number, 1, profile, name=prefix, instance_profile=instance_profile,
                                  availabilty_zone=availability_zone, monitoring=True, kp_dir=self.kp_dir)
            else:
                self.launch_fleet(account_number, 1, profile, name=prefix,
                                  user_data=user_data[nn], instance_profile=instance_profile, availabilty_zone=availability_zone, monitoring=True, kp_dir=self.kp_dir)

    def setup_fleet(self, account_number, prefix, n_jobs, profile, instance_profile='', return_fid=True):
        assert account_number is not None
        fid = self.launch_fleet(account_number, n_jobs, profile, name=prefix,
                                instance_profile=instance_profile, monitoring=True, kp_dir=self.kp_dir, return_fid=return_fid)
        return fid
    ...
    def distribute_scripts_on_instances(self, instance_ids, scripts):
        for inum, iid in enumerate(instance_ids):
            clear_output()
            self.launch_instance(iid, instance_id=True,
                                 scripts=[scripts[inum]])

    def run_sloppy_distributed_jobs(self, account_num, prefix, n_jobs, profile, region, scripts, instance_profile='', boot_wait_time=5):

        fid = self.setup_fleet(account_num, prefix, n_jobs, profile,
                               instance_profile=instance_profile, return_fid=True)
